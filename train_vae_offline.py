...
import time
from vae_config import args_gridworld, args_point_robot_sparse, args_cheetah_vel, args_ant_semicircle_sparse, args_point_robot_barrier, args_point_robot_rand_params
...
def train(vae, dataset, goals, args):
    '''
    ...
    for iter_ in range(args.num_iters):
        ...
        if (iter_ + 1) % args.eval_interval == 0:
            if args.env_name != 'GridNavi-v2':  # TODO: eval for gridworld domain
                rewards_eval, reward_preds_eval, state_preds, tasks_indeces = eval_vae(dataset, vae, args)
                for task in range(10):
                    writer.add_figure('reward_prediction/task_{}'.format(task),
                                      ...
                                      total_updates)
                    #writer.add_figure('state_prediction/task_{}'.format(task),
                    #                  utl_eval.plot_state_pred_vs_goal(state_preds[task, :], goals[tasks_indeces[task]]),
                    #                  total_updates)
