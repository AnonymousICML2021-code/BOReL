...
def extract_goal_from_path(path):
    ...
    try:
        ...
        goal_desc = path.split('task_')[-1]
        goal_desc = goal_desc.strip('[]')
        goal = [goal_part.split(' ') for goal_part in goal_desc.split('_')]
        ...
        try:
            ...
            pass
        goal = [float(goal_part) for goal_part in goal]
    return goal
...
def load_trained_vae(vae, path):
    ...
    if vae.args.decode_reward:
                 paths['reward_decoder_path'] = get_model_path(path, model_name='reward_decoder')
    if vae.args.decode_state:
        paths['state_decoder_path'] = get_model_path(path, model_name='state_decoder')
...
def load_replaying_dataset(data_dir, args, num_tasks=None):
    ...
    for i, task in enumerate(tasks):
        ...
        policies_to_load = np.random.choice(len(all_policies), policies_per_task, replace=False)
        goals.append(extract_goal_from_path(all_dirs[task]))
        task_obs, task_actions, task_rewards, task_next_obs, task_terminals = [], [], [], [], []
        for j, policy in enumerate(policies_to_load):
            exp_dir = os.path.join(exps_dir, all_dirs[task], all_policies[policy])
            obs, actions, rewards, next_obs, terminals = load_transitions(exp_dir)
...
def mix_policy_rollouts(first_dataset, first_goals, second_dataset, second_goals, args, fraction=1.):
    if args.max_rollouts_per_task == 2:
        mix_until_time = int(args.trajectory_len / args.max_rollouts_per_task)
        num_rollouts = first_dataset[0][0].shape[1]
        num_rollots_to_mix = int(num_rollouts * fraction)
        for i, set in enumerate(first_dataset):
            loc, _ = np.where(second_goals == first_goals[i])
            loc = int(loc[0])
            set_to_mix_from = second_dataset[loc]
            rollouts_to_mix = np.random.choice(num_rollouts, num_rollots_to_mix, replace=False)
            rollouts_to_mix_from = np.random.choice(set_to_mix_from[0].shape[1], num_rollots_to_mix)
            for rollout_idx, policy in zip(rollouts_to_mix, rollouts_to_mix_from):
                set[0][:mix_until_time, rollout_idx, :] = set_to_mix_from[0][:mix_until_time, policy, :]
                set[1][:mix_until_time, rollout_idx, :] = set_to_mix_from[1][:mix_until_time, policy, :]
                set[2][:mix_until_time, rollout_idx, :] = set_to_mix_from[2][:mix_until_time, policy, :]
                set[3][:mix_until_time, rollout_idx, :] = set_to_mix_from[3][:mix_until_time, policy, :]
                set[4][:mix_until_time, rollout_idx, :] = set_to_mix_from[4][:mix_until_time, policy, :]

            print('Mixed {} datasets.'.format(i + 1))
    else:
        raise NotImplementedError
    return first_dataset, first_goals
