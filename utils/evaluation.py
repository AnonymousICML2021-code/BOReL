...
def plot_state_pred_vs_goal(state_preds, goal):
    fig, ax = plt.subplots()
    circle1 = plt.Circle((0, 0), 1, edgecolor='black', facecolor='w')
    door_width = np.sqrt(1 + 1 - 2 * np.cos((np.pi / 8) / 2))
    x, y = np.cos(goal), np.sin(goal)
    circle2 = plt.Circle((x, y), radius=door_width, alpha=0.3)
    ...
    obs = np.vstack([obs_x, obs_y]).transpose()

    im = ax.scatter(obs[:, 0], obs[:, 1], c=state_preds, zorder=10)
    fig.colorbar(im)
