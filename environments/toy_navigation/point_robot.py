...
class PointEnvRandDynamics(PointEnv):
    def __init__(self,
                 max_episode_steps=20,
                 n_tasks=2,
                 modify_init_state_dist=True,
                 **kwargs):
        self.tasks = [None]
        super().__init__(max_episode_steps, n_tasks, modify_init_state_dist)

        radius = 1.0
        angle = np.pi/2
        x = radius * np.cos(angle)
        y = radius * np.sin(angle)
        self.goal_reward = np.array((x, y))
        self.goal_radius = 0.2

        # tasks are random "wind" to add to all state transitions
        self.tasks = [np.array((np.random.uniform(-0.05, 0.05),
                                     np.random.uniform(-0.05, 0.05)))
                      for _ in range(n_tasks)]
        # self.tasks = [np.array((0., 0.))
        #               for _ in range(n_tasks)]
        self.reset_task(0)
    ...
    def step(self, action):
        self._state = self._state + action + self._task
        reward = - ((self._state[0] - self.goal_reward[0]) ** 2 +
                    (self._state[1] - self.goal_reward[1]) ** 2) ** 0.5
        # make sparse rewards positive
        if reward >= -self.goal_radius:
            sparse_reward = 1.
        else:
        ...
        ob = self._get_obs()
        return ob, sparse_reward, done, dict()

    def reward(self, state, action=None):
        reward = - ((state[0] - self.goal_reward[0]) ** 2 +
                    (state[1] - self.goal_reward[1]) ** 2) ** 0.5
        if reward >= -self.goal_radius:
            sparse_reward = 1.
        else:
            sparse_reward = 0.
    ...
    def is_goal_state(self):
        if np.linalg.norm(self._state - self.goal_reward) <= self.goal_radius:
            return True
        else:
            return False
    ...
    def plot_env(self):
        ...
        plt.xticks([])
        plt.yticks([])
        circle = plt.Circle((self.goal_reward[0], self.goal_reward[1]),
                            radius=self.goal_radius, alpha=0.3)
        ax.add_artist(circle)
        plt.axis('scaled')
