...
class OfflineMetaLearner:
    ...
    def evaluate(self):
        ...
        state_preds = np.zeros((num_tasks, self.args.trajectory_len, circle_states.shape[0]))
        ...
        for task in self.env.unwrapped.get_all_task_idx():
            ...
            for episode_idx in range(num_episodes):
                ...
                for step_idx in range(num_steps_per_episode):
                    ...
                    prediction = ptu.get_numpy(self.vae.state_decoder(task_sample.expand((1, 30, task_sample.shape[-1])),
                                                                 ptu.FloatTensor(circle_states).unsqueeze(0),
                                                                 ptu.FloatTensor(circle_actions).unsqueeze(0))).squeeze()
                    for i in range(30):
                        state_preds[task, step, i] = 1 if np.linalg.norm(prediction[i, :]) > 1 else 0
    ...
    def log(self, iteration, train_stats):
        ...
        if iteration % self.args.log_interval == 0:
            if self.args.policy == 'dqn':
                returns, success_rate, observations, rewards, reward_preds, state_preds = self.evaluate()
            ...
            #     rewards, reward_preds, reward_belief, reward_belief_discretized, points = self.evaluate()
                ...
                returns, success_rate, log_probs, observations, rewards, reward_preds, state_preds = self.evaluate()
                ...
                for i, task in enumerate(tasks_to_vis):
                    ...
                    self.tb_logger.writer.add_figure('reward_prediction_train/task_{}'.format(i),
                                                     utl_eval.plot_rew_pred_vs_rew(rewards[task, :],
                                                                                   reward_preds[task, :]),
                                                     self._n_rl_update_steps_total)
                #for i in range(state_preds.shape[1]):
                    #if (i+1)%30==0:
                    #    self.tb_logger.writer.add_figure('state_prediction_train/step_{}'.format(i),
                    #                                     utl_eval.plot_state_pred_vs_goal(state_preds[0, i, :], self.goals[self.env.unwrapped.get_all_task_idx()[task]]),
                    #                                     self._n_rl_update_steps_total)
                    #self.tb_logger.writer.add_figure('reward_prediction_train/task_{}'.format(i),
    ...
    def collect_rollouts(self):
        ...
        rewards = np.zeros((num_tasks, self.args.trajectory_len))
        observations = np.zeros((num_tasks, self.args.trajectory_len + 1, obs_size))
        actions = np.zeros((num_tasks, self.args.trajectory_len, self.args.action_dim))
...
def get_heatmap_params():
    angles = np.linspace(0, np.pi, 30)
    radius = 0.99
    action_radius = 0.1
    obs_x = radius * np.cos(angles)
    obs_y = radius * np.sin(angles)
    done = np.zeros_like(obs_x)
    obs = np.vstack([obs_x, obs_y, done]).transpose()
    action_x = action_radius * np.cos(angles)
    action_y = action_radius * np.sin(angles)
    actions = np.vstack([action_x, action_y]).transpose()
    return obs, actions
